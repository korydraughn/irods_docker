...
counter=0
until pg_isready -h irods-tempzone-catalog -d ICAT -U irods -q
do
    sleep 1
    ((counter += 1))
done
...
su - irods -c 'iadmin mkzone otherZone remote irods-otherzone:1247'

# Create an additional rodsuser for federation.
su - irods -c 'iadmin mkuser otherrods#otherZone rodsuser && iadmin moduser otherrods#otherZone password rods'
