...
#acAclPolicy {msiAclPolicy("STANDARD"); }
#
# --------------------------------------------------------------------------
...
#pep_api_data_obj_put_post(*ARG0,*ARG1,*ARG2,*ARG3,*ARG4) {  }
...
#pep_database_mod_data_obj_meta_pre(*ctx, *data_obj_info, *reg_param) {}
#pep_database_mod_data_obj_meta_pre() {}
#pep_database_mod_data_obj_meta_pre(*a, *b, *c, *d, *e) { writeLine("serverLog", "pep_database_mod_data_obj_meta_pre fired!!!"); }

#pep_database_open_pre(*inst, *ctx, *out) { writeLine("serverLog", "pep_database_open_pre triggered!!!"); }

my_delay_test() {
   delay("<PLUSET>1s</PLUSET><EF>1s</EF>") {
      writeLine("stdout", "-- STDOUT: Delayed Execution --");
      writeLine("stderr", "-- STDERR: Delayed Execution --");
      writeLine("serverLog", "-- SERVERLOG: Delayed Execution --");
      writeLine("INVALID_OUTPUT_ID", "-- SERVERLOG: Delayed Execution --");
   }
}
...
{
    foreach (*k in *obj) {
        writeLine("serverLog", ">>>> PRINT_KEYS :: key = " ++ *k);
    }
}

get_parent_collection(*path)
{
    msiSplitPath(*path, *out, *_);
    *out
}

get_tracked_parent_collection(*path)
{
    *p = get_parent_collection(*path);
    *tracked_path = ""

    while (*p != "/") {
        if (is_tracking_object_count_and_size(*p)) {
            *tracked_path = *p;
            break;
        }
...
is_sub_collection_of(*child_path, *parent_path)
{
    "*child_path" like "*parent_path/.+"
}

calc_object_count_and_size(*path, *count, *size)
{
    *count = 0;
    *size = 0;

    # Compute the total number of data objects and the sum of the data object sizes under "*path".
    foreach (*row in select count(DATA_NAME), sum(DATA_SIZE) where COLL_NAME = "*path" || like "*path/%") {
        *count = int(*row.DATA_NAME);
        *size = int(*row.DATA_SIZE);
    }

    # Compute the total number of collections under "*path".
    foreach (*row in select count(COLL_NAME) where COLL_NAME like "*path/%") {
        *count = *count + int(*row.COLL_NAME);
    }
}

init_object_count_and_size(*path, *max_number_of_objects, *max_size_in_bytes)
{
    *count = 0;
    *size = 0;

    # Compute the total number of data objects and the sum of the data object sizes under "*_path".
    foreach (*row in select count(DATA_NAME), sum(DATA_SIZE) where COLL_NAME = "*path" || like "*path/%") {
        *count = int(*row.DATA_NAME);
        *size = int(*row.DATA_SIZE);
    }

    # Compute the total number of collections under "*_path".
    foreach (*row in select count(COLL_NAME) where COLL_NAME like "*path/%") {
        *count = *count + int(*row.COLL_NAME);
    }
    ...
    *ec = msiSetKeyValuePairsToObj(*kvp, *path, "-C");

    *ec;
}

is_tracking_object_count_and_size(*path)
{
    *is_tracking = false;

    foreach (*row in select META_COLL_ATTR_NAME where COLL_NAME = "*path" and META_COLL_ATTR_NAME = "mcount") {
        *is_tracking = true;
    }
...
data_object_exists(*path)
{
    msiSplitPath(*path, *coll_name, *data_name);
...
get_object_count_and_size(*path)
{
    foreach (*row in select META_COLL_ATTR_NAME, META_COLL_ATTR_VALUE where COLL_NAME = "*path") {
...
fail_if_max_count_violation(*kvp, *delta)
{
    if (int(*kvp.ccount) + *delta > int(*kvp.mcount)) {
        failmsg(-1, "Policy Violation: Adding object exceeds max number of objects limit");
    }
}

fail_if_max_size_violation(*kvp, *delta)
{
    if (int(*kvp.csize) + *delta > int(*kvp.msize)) {
        failmsg(-1, "Policy Violation: Adding objects exceeds max number of bytes limit");
    }
}

add_number_of_objects_and_size(*kvp, *num_objects, *size_in_bytes)
{
    *kvp.ccount = str(int(*kvp.ccount) + *num_objects);
    *kvp.csize = str(int(*kvp.csize) + *size_in_bytes);
}

pep_api_coll_create_pre(*INSTANCE_NAME, *COMM, *INPUT)
{
    *parent_path = get_tracked_parent_collection(*INPUT.coll_name);

    if (*parent_path != "") {
        *kvp = get_object_count_and_size(*parent_path);
        fail_if_max_count_violation(*kvp, 1, 0);
        add_number_of_objects_and_size(*kvp, 1, 0);
        msiSetKeyValuePairsToObj(*kvp, *parent_path, "-C");
    }
}
    ...
    *parent_path = get_tracked_parent_collection(*INPUT.coll_name);
    ...
    if (*parent_path != "") {
        *kvp = get_object_count_and_size(*parent_path);
        ...
        calc_object_count_and_size(*INPUT.coll_name, *count, *size);
        add_number_of_objects_and_size(*kvp, -int(*count + 1), *size);
...
{
    *src_parent_path = get_tracked_parent_collection(*INPUT.src_obj_path);
    *dst_parent_path = get_tracked_parent_collection(*INPUT.dst_obj_path);

    if (*src_parent_path != *dst_parent_path && !is_sub_collection_of(*dst_parent_path, *src_parent_path)) {
        # Must check the destination first because incrementing the number of
        # objects and/or size could violate the policy.
        if (*dst_parent_path != "") {
            *kvp = get_object_count_and_size(*dst_parent_path);

            msiGetObjType(*INPUT.src_obj_path, *obj_type);

            if (*obj_type == "-d") {
                *src_data_size = get_data_object_size(*INPUT.src_obj_path);

                fail_if_max_count_violation(*kvp, 1);
                fail_if_max_size_violation(*kvp, *src_data_size);

                add_number_of_objects_and_size(*kvp, 1, *src_data_size);

                msiSetKeyValuePairsToObj(*kvp, *dst_parent_path, "-C");
            }
            else if (*obj_type == "-c") {
                calc_object_count_and_size(*INPUT.src_obj_path, *count, *size);

                # Must add one for the collection that's being moved.
                *count = *count + 1;

                fail_if_max_count_violation(*kvp, *count);
                fail_if_max_size_violation(*kvp, *size);

                add_number_of_objects_and_size(*kvp, *count, *size);
        ...
        if (*src_parent_path != "") {
            *kvp = get_object_count_and_size(*src_parent_path);

            msiGetObjType(*INPUT.src_obj_path, *obj_type);

            if (*obj_type == "-d") {
                *src_data_size = get_data_object_size(*INPUT.src_obj_path);
                add_number_of_objects_and_size(*kvp, -1, -int(*src_data_size));
                msiSetKeyValuePairsToObj(*kvp, *src_parent_path, "-C");
            }
            else if (*obj_type == "-c") {
                calc_object_count_and_size(*INPUT.src_obj_path, *count, *size);
                add_number_of_objects_and_size(*kvp, -int(*count + 1), -*size);
...
pep_api_data_obj_create_pre(*INSTANCE_NAME, *COMM, *INPUT)
{
    print_keys(*INPUT);
}
...
{
    *parent_path = get_tracked_parent_collection(*INPUT.obj_path);

    if (*parent_path != "" && !data_object_exists(*INPUT.obj_path)) {
        *kvp = get_object_count_and_size(*parent_path);

        fail_if_max_count_violation(*kvp, 1);
        fail_if_max_size_violation(*kvp, int(*INPUT.data_size));

        add_number_of_objects_and_size(*kvp, 1, int(*INPUT.data_size));
...
pep_api_data_obj_unlink_pre(*INSTANCE_NAME, *COMM, *INPUT)
{
    *parent_path = get_tracked_parent_collection(*INPUT.obj_path);
    ...
    if (*parent_path != "") {
        *kvp = get_object_count_and_size(*parent_path);
        add_number_of_objects_and_size(*kvp, -1, -get_data_object_size(*INPUT.obj_path));
        msiSetKeyValuePairsToObj(*kvp, *parent_path, "-C");
    }
}
